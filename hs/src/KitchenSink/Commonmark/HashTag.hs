...
hashtagSpec :: (Monad m, IsBlock il bl, IsInline il, HasHashTag il)
          => SyntaxSpec m il bl
hashtagSpec = mempty
...
class HasHashTag a where
  hashtag :: Text   -- the hashtag value
          -> a

instance {-# OVERLAPPABLE #-} HasHashTag (Html a) where
  hashtag kw = addAttribute ("class", "hashtag") .
                addAttribute ("data-hashtag", kw) $
    htmlInline "span" $ Just $ htmlText $ "#" <> kw
